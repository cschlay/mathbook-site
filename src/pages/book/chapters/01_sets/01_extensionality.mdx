...
import {ColorPaletteExample} from "../../../../components/examples/ColorPaletteExample";
...
<Quoted>
  ...
  - If <tex>x</tex> is an element of a set <tex>S</tex>, we write <tex value="x \in S"/>;
  - If not, we write <tex value="x \notin S"/>.
  - The set which has no elements is called _empty set_ and denoted <tex value="\emptyset"/>.
  ...
  In general, when we have objects <tex value="a_1, \ldots, a_n"/>, then <tex value="\{a_1, \ldots, a_n\}"/> is _the_ set <tex>S</tex> whose elements are <tex value="a_1, \ldots, a_n"/>.
...
<Quoted>
  ...
  We’ll use the following shorthand notation for that: <tex value="\{ x \mid f(x) \}," block={true} /> where the
  <tex>f(x)</tex> stands for the property that <tex>x</tex> has to have in order to be counted among
  the elements of the set. The property can be thought as a function that generate the elements.
  The symbol <tex value="\mid"/> is written as `\mid`.
...
<LatexExercise answer={/S = \\{ 2\^n \\mid n \\in \\mathbb{N} \\}/} slug="set-generator">
  <p>Define a set <tex>S</tex> in LaTeX that generates all <tex>2^n</tex> elements in the set of natural numbers <tex>{`\\mathbb{N}`}</tex>.</p>
</LatexExercise>