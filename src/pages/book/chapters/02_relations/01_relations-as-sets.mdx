...
The most basic relation is <em>identity relation</em> where elements relate themselves $(x, x) \in R$ when $x \in A$, denoted as $\text{Id}_A$.
...
Each row consist of pairs in $R$ such as $(\text{shorts}, \text{t-shirt})$ and elements are the columns.
If we were to define three columns, we would need to define relation of relations $((\text{shorts}, \text{t-shirt}), \text{sandals})$.
