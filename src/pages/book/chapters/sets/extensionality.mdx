...
<Definition name="Extensionality">
    If <Tex math="A"/> and <Tex math="B"/> are sets, then <Tex math="A = B"/> if and only if every element of <Tex math="A"/> is also an element of <Tex math="B"/>, and vice versa.
</Definition>
...
<Example name="Color palette">
    <p>
        Colors used in a website design can be thought as a set <Tex math="S = \{blue, green, orange\}"/>.
...
We’ll use the following shorthand notation for that: $\{x \mid f(x)\}$, where the
...
<Example name="Generating serving sizes">
    <p>
        ...
        It is decided to serve them in multiples of <Tex math="1.5"/> where base amount is <Tex math="100\text{ g}"/>.
        But no more than <Tex math="500\text{ g}"/>. To generate all sizes we could specify:
    </p>
    <Tex math="S = \{ x \mid x \cdot 1.5 \le 500 \} = \{ 150, 225, 337.5 \}." block/>
</Example>
...
<Problem>
    A multiple choice problem to select equal sets.
</Problem>